...
public class DropContents implements SQLAction {
    ...
    public DropContents(String database, String schema) {
        ...
        for (String table : TableConstants.TABLES) {
            sql.append("DROP TABLE ").append(table).append(" CASCADE;\n");
        }
        sql.append("DROP SCHEMA ").append(this.schema).append(";\n");
        code = sql.toString();
    }
    ...
    public String getFunction() {
        return "Drop the database " + database + "'s contents including the schema: "+this.schema;
    }
}
